...
local function _unserializePhp(data, offset)
    ...
    elseif dtype == 'a' then
        ...
        for i = 0, keys - 1, 1 do
            kprops = _unserializePhp(data, dataoffset);

            kchrs  = tonumber(kprops[2])
            key    = kprops[3]
            dataoffset = dataoffset + kchrs

            vprops = _unserializePhp(data, dataoffset)
...
local function _unserializeSession(data)
    ...
    repeat
        ...
        local r = _unserializePhp(data, pos)
...
function unserializer.unserializePhp(data)
    local result = _unserializePhp(data or "", 1)
    if result ~= nil then
        return result[3]
    end
    return nil
